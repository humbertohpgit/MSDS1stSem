...
### More men have smoked that women
...
```{r boxplot-bmi2, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
boxplot(bmi ~ cdc$exerany)
```
### Better BMI for people exercising
...
```{r wt_vs_wtdesire, eval=TRUE}
plot(cdc$weight, cdc$wtdesire)
abline(0,1, col="red")
```
#### Most of the population weight was in line with their desired weight up to ~250lbs. From there, the deviation started with actual weights from 250lbs to 400lbs, where the desired weight was between 200 to 300 max 
...
is(wdiff)
```
#### wdiff is an integer, if wdiff is zero it means the individual' actual weight matches the desired one. If positive means the individual is underweight, if negative, then is overweight
...
```{r wdiff_distrib, eval=TRUE}
hist(wdiff)
abline(v=mean(wdiff), col='red')
boxplot(wdiff)
```

#### wdiff shows a Normal distribution with the majority of the observations around the mean and +/- 1 SD

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r wtdesire_by_gender, eval=TRUE}
summary(cdc$wtdesire[cdc$gender=="m"])
summary(cdc$wtdesire[cdc$gender=="f"])
boxplot(cdc$wtdesire ~ cdc$gender)
```

#### Females tend to see their desired weight much lower than men, so that they always feel off from it as opposed to men who feel very close to it
...
```{r weight_mean_sd, eval=TRUE}
wtmean <- mean(cdc$weight)
wtmean
wtsd <- sd(cdc$weight)
wtsd
wtwithin1sd <- subset(cdc, (weight <= (wtmean+wtsd) & weight >= (wtmean-wtsd)))
nrow(wtwithin1sd)
perc <- (nrow(wtwithin1sd)/nrow(cdc))*100
perc
hist(cdc$weight)
x <- cdc$weight
y <- dnorm(x, mean = wtmean, sd = wtsd)
plot(x, y, type = 'h', xlab = 'Weight')
abline(v=wtmean, col='blue')
abline(v=wtmean+wtsd, col='red')
abline(v=wtmean-wtsd, col='red')
```
