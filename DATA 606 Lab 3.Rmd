---
title: "DATA 606 Lab 3 - The normal distribution"
author: "humbertohp"
...
```{r hist male & female, eval=TRUE}
hist(mdims$hgt)
hist(fdims$hgt)
```

#### Men's histogram looks like a Normal Distribution, Women's looks somewhat off a Normal Distribution, more like a Skewed Distribution
...
#### Very close but it looks more like a Skewed Distribution
...
```{r qq sim_norm, eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)
```

#### Not all the points fall on the line, it looks somewhat similar to the probability plot of the real female height data mainly on the line ends. However, they differ more at the center of the line with the real data following a "zig-zag"/stepwise pattern while the simulated data looks smoother along the line
...
#### The same pattern is confirmed as with the first simulation described above - Very similar at the line ends but a "zig-zag"/stepwise pattern along the center, as opposed to the smoother patterns of the simulations
...
#### Female weights appear to follow more closely a Normal Distribution, smoother pattern along the line
...
#### 1) Probability that someone is shorter than 182 cm?
...
#### 1) Probability that someone weighs more than 65 kg?
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter 
    
#### Plot B

```{r hist bii.di, eval=TRUE}
...
```
    ...
    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter

#### Plot C

```{r hist elb.di, eval=TRUE}
...
hist(bdims$age, probability = TRUE)
qqnorm(bdims$age)
qqline(bdims$age)
```
...
#### Data have been rounded or are discrete. Similar to hgt, both seem to have a more Skewed distribution
