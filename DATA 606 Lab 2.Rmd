---
title: "DATA 606 Lab 2 - Probability"
author: "humbertohp"
...
#### A streak of 1 means Kobe did 1 hit max and then he missed. A streak of 0 means no hits, only misses
...
#### The typical streak length was 0 and 1. Longest streak was 4 baskets
...
```{r calc-streak-indshooter, eval=TRUE}
...
```{r table-streak-indshooter, eval=TRUE}
table(indshooter_streak)
```

#### The typical streak length of an independent shooter was 0 and 1. Longest streak was 5 or 6 baskets (depending on the sim run)
...
#### Somewhat similar as the conditions remain the same: sample size, shooting % or Probability
```{r calc-streak-indshooter2, eval=TRUE}
outcomes <- c("H", "M")
sim_basket2 <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.45, 0.55))
indshooter_streak2 <- calc_streak(sim_basket2)
table(indshooter_streak2)
```
#### Now with the second simulation the typical streak length of an independent shooter was also 0 and 1, but now the longest streak was 7 baskets
...
```{r compare-streaks, eval=TRUE}
table(kobe_streak)
table(indshooter_streak)
```

#### Very similar distributions, where the typical streak length was 0 and 1 with just a variation on the longest streak (Kobel=4, IndShooter=5 or 6 (depending on the sim run))
#### Conclusion: With the analysis performed, there is no evidence that the hot hand model fits Kobe's shooting patterns as the streak lengths between Kobe and the IndShooter were very similar, given that the principle for the latter was all about independence of shots
