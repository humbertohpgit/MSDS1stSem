...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    How would you describe it? - Consistent Linear growth of girls baptized with a noticeable drop approximately in year 1720 and back up again right after
...
```{r compare-data, eval=TRUE}
dim(arbuthnot)
dim(present)
```
##### Present data set is larger, 19 more records
...
```
##### There was a decline in the boys/girls proportion in the US
...
```{r max_births, eval=TRUE}
attach(present)
present[order(-boys,-girls),] #Option1
present[which.max(present$boys+present$girls),] #Option2
```
##### Year:1961
